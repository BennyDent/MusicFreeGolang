...
func (env *Env) Musician_For_Search(w http.ResponseWriter, req *http.Request) {

	var page_index, page_size, is_error = PagesStringToNumbers(req, w)
	if !is_error {
		return
	}
	var result, error = env.DB.QueryContext(req.Context(), `SELECT authors.id, authors.name, authors.cover_filename 
FROM authors
INNER JOIN songs ON songs.id = authors.id
INNER JOIN authors_to_songs ON authors.id = authors_to_songs.author_id
WHERE authors.name LIKE '%$1%'
GROUP BY authors.name,authors.id, authors.cover_filename 
ORDER BY COUNT(songs.id), COUNT(authors_to_songs.id)
LIMIT $2 OFFSET $3`, req.PathValue("name"), page_size, page_index*page_size)
	...
	if PageSearchReturn(data, page_counter == page_size, w) {
		return
	}
}

func (env *Env) Tags_Genres__For_Search(w http.ResponseWriter, req *http.Request) {
	...
	var result, error = env.DB.QueryContext(req.Context(), `SELECT  $1s.name
FROM $1s
...
func PageSearchReturn[T any](data []T, hasMore bool, w http.ResponseWriter, opt ...bool) bool {
	type PageReturn struct {
		hasMore  bool
		data     []T
		optional any
	}
	var for_return PageReturn
	for_return.data = data
	for_return.hasMore = hasMore
	if len(opt) > 0 {
		for_return.optional = opt[0]
	}
